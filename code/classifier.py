...
# the first line of the raw data text is the ordered list of surrounding words
surr_words = data_text_raw[0].split(' ');
...
vector_size = len(vector_template);
...
clf = LogisticRegression(solver='newton-cg', max_iter=2000, random_state=0, \
    multi_class='multinomial', verbose=1).fit(training_data_predictors, \
    training_data_labels);

print(clf.score(unclassified_data, unclassified_data_labels));
